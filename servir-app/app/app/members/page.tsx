...
import { Plus, Pencil, Trash2, Phone, Search, LayoutGrid, List, Cake } from "lucide-react";

const FUNCOES: Funcao[] = ["Líder", "Co-líder", "Voluntário"];

const FUNCAO_COLORS: Record<Funcao, string> = {
  "Líder": "bg-blue-100 text-blue-700",
  "Co-líder": "bg-purple-100 text-purple-700",
...
const empty = { name: "", phone: "", teamId: "", funcao: "Voluntário" as Funcao, aniversario: "", active: true };
...
export default function MembersPage() {
  ...
  function openEdit(m: Member) {
    setEditing(m);
    setForm({ name: m.name, phone: m.phone, teamId: m.teamId, funcao: m.funcao ?? "Voluntário", aniversario: m.aniversario ?? "", active: m.active });
    setModalOpen(true);
  }
  ...
  // Agrupar por equipe para view de cards
  ...
  })).filter((g) => g.members.length > 0 || !filterTeam);
      ...
      <div className="flex gap-2 flex-wrap">
        ...
        <div className="flex border border-gray-200 rounded-lg overflow-hidden">
          <button
            ...
            className={`px-3 py-2 transition-colors ${view === "cards" ? "bg-blue-600 text-white" : "text-gray-400 hover:bg-gray-50"}`}
          >
            <LayoutGrid size={16} />
          </button>
          <button
            onClick={() => setView("list")}
            className={`px-3 py-2 transition-colors ${view === "list" ? "bg-blue-600 text-white" : "text-gray-400 hover:bg-gray-50"}`}
          >
            <List size={16} />
          </button>
        </div>
